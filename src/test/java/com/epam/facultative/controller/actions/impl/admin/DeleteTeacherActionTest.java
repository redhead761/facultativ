...
class DeleteTeacherActionTest {
    ...
    void executeSuccessful() throws ValidateException, ServiceException {
        setUpReturn();
        doNothing().when(adminService).deleteTeacher(anyInt());
        String path = new DeleteTeacherAction(appContext).execute(req, resp);
        verify(httpSession, times(1)).setAttribute(MESSAGE, SUCCESSFUL);
        assertEquals(getGetAction(MANAGE_TEACHERS_ACTION, CURRENT_PAGE, "1", RECORDS_PER_PAGE, "1"), path);
    }

    @Test
    void executeWithValidateException() throws ValidateException, ServiceException {
        setUpReturn();
        doThrow(ValidateException.class).when(adminService).deleteTeacher(anyInt());
        String path = new DeleteTeacherAction(appContext).execute(req, resp);
